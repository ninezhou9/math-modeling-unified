...
def batch_replace(
    ...
    *,
    ...
    if source == output:
        raise ValueError("output 必须与输入 DOCX 使用不同路径，拒绝原地覆盖")
    if output.exists() and not overwrite:
        raise FileExistsError(f"输出文件已存在；如需替换请显式传入 --overwrite: {output}")
    doc = Document(source)
    ...
    doc.save(output)
    print(f"\n完成: {success}/{len(mapping)} 个公式已插入 -> {output}")
    return success
...
def build_parser():
    ...
    rp.add_argument("--output", "-o", default=None,
                    help="必填：与输入不同的输出 .docx 路径")
    rp.add_argument("--overwrite", action="store_true",
                    help="显式覆盖已存在的不同输出文件；始终禁止覆盖输入文件")
...
def main():
    ...
    if args.mode == "replace":
        ...
        if not args.output:
            print(json.dumps({
                "status": "error",
                "error": "replace 必须显式提供 --output，且 output 必须与输入不同",
            }, ensure_ascii=False))
            return 1
        if Path(args.input).resolve() == Path(args.output).resolve():
            print(json.dumps({
                "status": "error",
                "error": "output 必须与输入 DOCX 使用不同路径；即使传入 --overwrite 也拒绝原地覆盖",
            }, ensure_ascii=False))
            return 1
        try:
            batch_replace(
