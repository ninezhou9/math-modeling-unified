...
SHARED_DOCX_SCRIPTS = Path(__file__).resolve().parents[3] / "scripts"
if str(SHARED_DOCX_SCRIPTS) not in sys.path:
    sys.path.insert(0, str(SHARED_DOCX_SCRIPTS))

try:  # defusedxml 兼容层：优先安全解析器，缺失时回退标准库
    from _defusedxml_compat import defusedxml
except ImportError:
    from office._defusedxml_compat import defusedxml

TEMPLATE_DIR = SHARED_DOCX_SCRIPTS / "templates"
NS = {
