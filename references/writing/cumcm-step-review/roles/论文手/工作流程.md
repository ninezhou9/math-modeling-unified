...
**分部分写作强制项**：每个论文部分写入正文前，读取 `references/优秀论文写法指南.md` 对应章节，并按该章节的篇幅、结构、句式与评委关注点组织正文（对照 `自审框架.md` 第三节）；本 Skill 的 10 部分审核顺序与成稿排版顺序见 `references/阶段流程.md`。

### Word 分支

1. 从当届官方参考模板创建文档；模板正文示例必须清除，但保留样式、分节、页眉页脚和页面设置。
2. 使用 `<SKILL_ROOT>/tools/docx/scripts/paper_format.py` 添加标题、段落、公式、表和图片。
...
用户要求的全部格式冻结且各自确定性门禁全部返回 0 后，需要可选独立质检时按统一[质量门禁](../../../../quality/quality-gates.md)执行，不得替代用户审核。质检者核对当届规则、主张—证据、数值与单位、图表编号和正文引用、文献及实际渲染效果；同时生成两种格式时再检查 Word/LaTeX 一致性。失败回执必须定位到页码、章节、命令或来源，并按根因返回对应角色。
