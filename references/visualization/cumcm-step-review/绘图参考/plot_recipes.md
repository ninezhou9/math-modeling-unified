...
```python
import sys, os
sys.path.insert(0, '<SKILL_ROOT>/scripts')   # 或把 plot_style.py 复制到 PROJECT_ROOT/utils 后从项目导入
...
def sig_bracket(ax, x1, x2, y, h, text, fontsize=8):
    """在 (x1, x2) 之间画一道带文字的桥。"""
...
# 图内不写标题（用户定制）：'Bimodal distribution' 等信息放图注
...
# 图内不写标题（用户定制）：'Right-skewed: mean vs median' 等信息放图注
ax.legend(frameon=False, fontsize=8)
...
ax.tick_params(labelsize=6)
# 图内不写标题（用户定制）：'Feature correlations' 等信息放图注
